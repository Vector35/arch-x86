...
class X86CommonArchitecture : public Architecture
{
 protected:
	...
	BNRegisterInfo RegisterInfo(
	    xed_reg_enum_t fullWidthReg, size_t offset, size_t size, bool zeroExtend = false);
	static void GetAddressSizeToken(
	...
	void GetInstructionPadding(
	    const unsigned int instruction_name_length, vector<InstructionTextToken>& result) const;
	void GetOperandTextIntel(const xed_decoded_inst_t* const xedd, const uint64_t addr,
	    const size_t len, const xed_operand_values_t* const ov, const xed_inst_t* const xi,
	    vector<InstructionTextToken>& result) const;
	void GetOperandTextBNIntel(const xed_decoded_inst_t* const xedd, const uint64_t addr,
	    const size_t len, const xed_operand_values_t* const ov, const xed_inst_t* const xi,
	    vector<InstructionTextToken>& result) const;
	void GetOperandTextATT(const xed_decoded_inst_t* const xedd, const uint64_t addr,
	    const size_t len, const xed_operand_values_t* const ov, const xed_inst_t* const xi,
	    vector<InstructionTextToken>& result) const;
	void GetOperandTextXED(const xed_decoded_inst_t* const xedd, const uint64_t addr, const size_t,
	    const xed_operand_values_t* const, const xed_inst_t* const,
	    vector<InstructionTextToken>& result) const;
	void GetOperandText(const xed_decoded_inst_t* const xedd, const uint64_t addr, const size_t len,
	    const xed_operand_values_t* const ov, const xed_inst_t* const xi,
	    vector<InstructionTextToken>& result) const;
 ...
 public:
	...
	virtual map<uint32_t, BNLowLevelILFlagCondition> GetFlagConditionsForSemanticFlagGroup(
	...
	virtual bool Assemble(
	    const string& code, uint64_t addr, DataBuffer& result, string& errors) override;
	...
	virtual bool IsSkipAndReturnZeroPatchAvailable(
	    const uint8_t* data, uint64_t, size_t len) override;
