...
enum COFFx86RelocationType : uint32_t
{
	...
	PE_IMAGE_REL_I386_SECTION = 0x000A,   // The 16-bit section index of the section that contains the
	                                      // target. This is used to support debugging information.
	PE_IMAGE_REL_I386_SECREL =
	    0x000B,  // The 32-bit offset of the target from the beginning of its section. This is used to
	...
	PE_IMAGE_REL_I386_SECREL7 =
	    0x000D,  // A 7-bit offset from the base of the section that contains the target.
	PE_IMAGE_REL_I386_REL32 = 0x0014,  // The 32-bit relative displacement to the target. This
...
enum COFFx64RelocationType : uint32_t
{
	...
	PE_IMAGE_REL_AMD64_REL32 =
	    0x0004,  // The 32-bit relative address from the byte following the relocation.
	PE_IMAGE_REL_AMD64_REL32_1 =
	    0x0005,  // The 32-bit address relative to byte distance 1 from the relocation.
	PE_IMAGE_REL_AMD64_REL32_2 =
	    0x0006,  // The 32-bit address relative to byte distance 2 from the relocation.
	PE_IMAGE_REL_AMD64_REL32_3 =
	    0x0007,  // The 32-bit address relative to byte distance 3 from the relocation.
	PE_IMAGE_REL_AMD64_REL32_4 =
	    0x0008,  // The 32-bit address relative to byte distance 4 from the relocation.
	PE_IMAGE_REL_AMD64_REL32_5 =
	    0x0009,  // The 32-bit address relative to byte distance 5 from the relocation.
	PE_IMAGE_REL_AMD64_SECTION = 0x000A,  // The 16-bit section index of the section that contains the
	                                      // target. This is used to support debugging information.
	PE_IMAGE_REL_AMD64_SECREL =
	    0x000B,  // The 32-bit offset of the target from the beginning of its section. This is used to
	             // support debugging information and static thread local storage.
	PE_IMAGE_REL_AMD64_SECREL7 =
	    0x000C,  // A 7-bit unsigned offset from the base of the section that contains the target.
	PE_IMAGE_REL_AMD64_TOKEN = 0x000D,  // CLR tokens.
	PE_IMAGE_REL_AMD64_SREL32 =
	    0x000E,  // A 32-bit signed span-dependent value emitted into the object.
	PE_IMAGE_REL_AMD64_PAIR =
	    0x000F,  // A pair that must immediately follow every span-dependent value.
	PE_IMAGE_REL_AMD64_SSPAN32 =
	    0x0010,  // A 32-bit signed span-dependent value that is applied at link time.
...
enum PeRelocationType : uint32_t
{
	PE_IMAGE_REL_BASED_ABSOLUTE =
	    0,  // The base relocation is skipped. This type can be used to pad a block.
	PE_IMAGE_REL_BASED_HIGH =
	    1,  // The base relocation adds the high 16 bits of the difference to the 16-bit field at
	        // offset. The 16-bit field represents the high value of a 32-bit word.
	PE_IMAGE_REL_BASED_LOW =
	    2,  // The base relocation adds the low 16 bits of the difference to the 16-bit field at
	        // offset. The 16-bit field represents the low half of a 32-bit word.
	PE_IMAGE_REL_BASED_HIGHLOW = 3,  // The base relocation applies all 32 bits of the difference to
	                                 // the 32-bit field at offset.
	PE_IMAGE_REL_BASED_HIGHADJ =
	    4,  // The base relocation adds the high 16 bits of the difference to the 16-bit field at
	        // offset. The 16-bit field represents the high value of a 32-bit word. The low 16 bits of
	        // the 32-bit value are stored in the 16-bit word that follows this base relocation. This
	        // means that this base relocation occupies two slots.
	PE_IMAGE_REL_BASED_MIPS_JMPADDR =
	    5,  // The relocation interpretation is dependent on the machine type. When the machine type
	        // is MIPS, the base relocation applies to a MIPS jump instruction.
	PE_IMAGE_REL_BASED_ARM_MOV32 = 5,  // This relocation is meaningful only when the machine type is
	                                   // ARM or Thumb. The base relocation applies the 32-bit address
	                                   // of a symbol across a consecutive MOVW/MOVT instruction pair.
	PE_IMAGE_REL_BASED_RISCV_HIGH20 =
	    5,  // This relocation is only meaningful when the machine type is RISC-V. The base relocation
	        // applies to the high 20 bits of a 32-bit absolute address.
	PE_IMAGE_REL_BASE_RESERVED = 6,  // Reserved, must be zero.
	PE_IMAGE_REL_BASED_THUMB_MOV32 =
	    7,  // This relocation is meaningful only when the machine type is Thumb. The base relocation
	        // applies the 32-bit address of a symbol to a consecutive MOVW/MOVT instruction pair.
	PE_IMAGE_REL_BASED_RISCV_LOW12I =
	    7,  // This relocation is only meaningful when the machine type is RISC-V. The base relocation
	        // applies to the low 12 bits of a 32-bit absolute address formed in RISC-V I-type
	        // instruction format.
	PE_IMAGE_REL_BASED_RISCV_LOW12S =
	    8,  // This relocation is only meaningful when the machine type is RISC-V. The base relocation
	        // applies to the low 12 bits of a 32-bit absolute address formed in RISC-V S-type
	        // instruction format.
	PE_IMAGE_REL_BASED_MIPS_JMPADDR16 =
	    9,  // The relocation is only meaningful when the machine type is MIPS. The base relocation
	        // applies to a MIPS16 jump instruction.
	PE_IMAGE_REL_BASED_DIR64 =
	    10,  // The base relocation applies the difference to the 64-bit field at offset.
	MAX_PE_RELOCATION
};
...
{
	static const char* relocTable[] = {"PE_IMAGE_REL_BASED_ABSOLUTE", "PE_IMAGE_REL_BASED_HIGH",
	    "PE_IMAGE_REL_BASED_LOW", "PE_IMAGE_REL_BASED_HIGHLOW", "PE_IMAGE_REL_BASED_HIGHADJ",
	    "PE_IMAGE_REL_BASED_MIPS_JMPADDR", "PE_IMAGE_REL_BASED_ARM_MOV32",
	    "PE_IMAGE_REL_BASED_RISCV_HIGH20", "PE_IMAGE_REL_BASE_RESERVED",
	    "PE_IMAGE_REL_BASED_THUMB_MOV32", "PE_IMAGE_REL_BASED_RISCV_LOW12I",
	    "PE_IMAGE_REL_BASED_RISCV_LOW12S", "PE_IMAGE_REL_BASED_MIPS_JMPADDR16",
	    "PE_IMAGE_REL_BASED_DIR64"};
...
{
	static const char* relocTable[] = {"R_X86_64_NONE", "R_X86_64_64", "R_X86_64_PC32",
	    "R_X86_64_GOT32", "R_X86_64_PLT32", "R_X86_64_COPY", "R_X86_64_GLOB_DAT",
	    "R_X86_64_JUMP_SLOT", "R_X86_64_RELATIVE", "R_X86_64_GOTPCREL", "R_X86_64_32", "R_X86_64_32S",
	    ...
	    "R_X86_64_GOTOFF64", "R_X86_64_GOTPC32", "R_X86_64_UNKNOWN27", "R_X86_64_UNKNOWN28",
	    "R_X86_64_UNKNOWN29", "R_X86_64_UNKNOWN30", "R_X86_64_UNKNOWN31", "R_X86_64_SIZE32",
	    "R_X86_64_SIZE64", "R_X86_64_GOTPC32_TLSDESC", "R_X86_64_TLSDESC_CALL", "R_X86_64_TLSDESC",
	    "R_X86_64_IRELATIVE", "R_X86_64_RELATIVE64", "R_X86_64_PC32_BND", "R_X86_64_PLT32_BND",
...
{
	static const char* relocTable[] = {"X86_64_RELOC_UNSIGNED", "X86_64_RELOC_SIGNED",
	    "X86_64_RELOC_BRANCH", "X86_64_RELOC_GOT_LOAD", "X86_64_RELOC_GOT", "X86_64_RELOC_SUBTRACTOR",
...
void X86CommonArchitecture::SetInstructionInfoForInstruction(
    uint64_t addr, InstructionInfo& result, xed_decoded_inst_t* xedd)
{
	result.length = xed_decoded_inst_get_length(xedd);

	const uint64_t abs_br =
...
BNRegisterInfo X86CommonArchitecture::RegisterInfo(
    xed_reg_enum_t fullWidthReg, size_t offset, size_t size, bool zeroExtend)
{
...
void X86CommonArchitecture::GetAddressSizeToken(
    const short bytes, vector<InstructionTextToken>& result, const bool lowerCase)
{
	// Size
	switch (bytes)
	{
...
unsigned short X86CommonArchitecture::GetInstructionOpcode(const xed_decoded_inst_t* const xedd,
...
void X86CommonArchitecture::GetInstructionPadding(
    const unsigned int instruction_name_length, vector<InstructionTextToken>& result) const
{
	string padding = "";
...
void X86CommonArchitecture::GetOperandTextIntel(const xed_decoded_inst_t* const xedd,
    const uint64_t addr, const size_t len, const xed_operand_values_t* const ov,
    const xed_inst_t* const xi, vector<InstructionTextToken>& result) const
{
	...
	for (unsigned int opIndex = 0; opIndex < xed_inst_noperands(xi); ++opIndex)
	{
		...
		{
			GetAddressSizeToken(xed_decoded_inst_operand_length_bits(xedd, opIndex) / 8, result,
			    m_disassembly_options.lowerCase);
			...
			const bool validSegment =
			    (seg != XED_REG_INVALID && !xed_operand_values_using_default_segment(ov, 0));
			if (validSegment)
			{
			...
			const unsigned short disp_bytes =
			    xed_decoded_inst_get_memory_displacement_width_bits(xedd, 0) / 8;
			...
			else if (xed_operand_values_has_memory_displacement(ov) && ((disp == 0) && (!started)))
			{
		...
		case XED_OPERAND_IMM1:  // The ENTER instruction
		{
			...
			result.emplace_back(
			    IntegerToken, sstream.str(), xed_decoded_inst_get_second_immediate(xedd), 1);
			break;
		}
		case XED_OPERAND_PTR:  // TODO...remove?
		{
			...
			result.emplace_back(
			    PossibleAddressToken, sstream.str(), xed_decoded_inst_get_branch_displacement(xedd), 8);
			break;
		}
		...
		if ((opIndex != xed_inst_noperands(xi) - 1) &&
		    ((xed_operand_operand_visibility(xed_inst_operand(xi, opIndex + 1)) ==
		         XED_OPVIS_EXPLICIT) ||
		        (xed_operand_operand_visibility(xed_inst_operand(xi, opIndex + 1)) ==
		            XED_OPVIS_IMPLICIT)))
			result.emplace_back(OperandSeparatorToken, m_disassembly_options.separator);
...
// The syntax used by asmx86, that users are used to (also the only one that should be garenteed to
// roundtrip with asm)
void X86CommonArchitecture::GetOperandTextBNIntel(const xed_decoded_inst_t* const xedd,
    const uint64_t addr, const size_t len, const xed_operand_values_t* const ov,
    const xed_inst_t* const xi, vector<InstructionTextToken>& result) const
{
	xed_reg_enum_t extra_index_operand = XED_REG_INVALID;
	...
	for (unsigned int opIndex = 0; opIndex < xed_inst_noperands(xi); ++opIndex)
	{
		...
		if ((xed_operand_operand_visibility(op) == XED_OPVIS_IMPLICIT) &&
		    (xed_decoded_inst_get_reg(xedd, op_name) == XED_REG_ST0))
			continue;
		...
		case XED_OPERAND_RELBR:
		{
			...
			if ((xed_decoded_inst_get_iclass(xedd) == XED_ICLASS_CALL_NEAR) &&
			    (relbr == (int64_t)(addr + xed_decoded_inst_get_length(xedd))))
			{
		    ...
		    ((xed_operand_operand_visibility(xed_inst_operand(xi, opIndex + 1)) ==
		        ...
		        ((xed_operand_operand_visibility(xed_inst_operand(xi, opIndex + 1)) ==
		            ...
		            (xed_decoded_inst_get_reg(
		                 xedd, xed_operand_name(xed_inst_operand(xi, opIndex + 1))) != XED_REG_ST0))))
			result.emplace_back(OperandSeparatorToken, m_disassembly_options.separator);
	}
...
void X86CommonArchitecture::GetOperandTextATT(const xed_decoded_inst_t* const xedd,
    const uint64_t addr, const size_t len, const xed_operand_values_t* const ov,
    const xed_inst_t* const xi, vector<InstructionTextToken>& result) const
{
	unsigned i, j;
	...
	for (j = 0; j < noperands; ++j)
	{
		...
		case XED_OPERAND_MEM0:
		{
			GetAddressSizeToken(xed_decoded_inst_operand_length_bits(xedd, i) / 8, result,
			    m_disassembly_options.lowerCase);
		...
		{
			if ((i != 0) && ((xed_operand_operand_visibility(xed_inst_operand(xi, noperands - j - 2)) ==
			                     XED_OPVIS_EXPLICIT) ||
			                    (xed_operand_operand_visibility(
			                         xed_inst_operand(xi, noperands - j - 2)) == XED_OPVIS_IMPLICIT)))
			{
				result.emplace_back(OperandSeparatorToken, m_disassembly_options.separator);
			}
		}
...
void X86CommonArchitecture::GetOperandTextXED(const xed_decoded_inst_t* const xedd,
    const uint64_t addr, const size_t, const xed_operand_values_t* const, const xed_inst_t* const,
    vector<InstructionTextToken>& result) const
{
	char out_buffer[100];
...
void X86CommonArchitecture::GetOperandText(const xed_decoded_inst_t* const xedd,
    const uint64_t addr, const size_t len, const xed_operand_values_t* const ov,
    const xed_inst_t* const xi, vector<InstructionTextToken>& result) const
{
	switch (m_disassembly_options.df)
...
X86CommonArchitecture::X86CommonArchitecture(const string& name, size_t bits) :
    Architecture(name), m_bits(bits)
{
...
{
	return vector<uint32_t> {XED_REG_CS, XED_REG_DS, XED_REG_ES, XED_REG_SS, XED_REG_FS, XED_REG_GS,
	    XED_REG_FSBASE, XED_REG_GSBASE, XED_REG_FLAGS, XED_REG_EFLAGS, XED_REG_RFLAGS};
}

vector<uint32_t> X86CommonArchitecture::GetSystemRegisters()
{
	return vector<uint32_t> {XED_REG_BND0, XED_REG_BND1, XED_REG_BND2, XED_REG_BND3,
	    ...
	    XED_REG_CR14, XED_REG_CR15,

	    XED_REG_DR0, XED_REG_DR1, XED_REG_DR2, XED_REG_DR3, XED_REG_DR4, XED_REG_DR5, XED_REG_DR6,
	    XED_REG_DR7,

	    XED_REG_GDTR, XED_REG_LDTR, XED_REG_IDTR, XED_REG_TR,

	    XED_REG_TSC, XED_REG_TSCAUX, XED_REG_MSRS};
}

bool X86CommonArchitecture::GetInstructionInfo(
...
bool X86CommonArchitecture::GetInstructionText(
...
bool X86CommonArchitecture::GetInstructionLowLevelIL(
...
size_t X86CommonArchitecture::GetFlagWriteLowLevelIL(BNLowLevelILOperation op, size_t size,
    ...
    LowLevelILFunction& il)
{
	...
	case LLIL_MULU_DP:
		...
		case IL_FLAG_C:
		case IL_FLAG_O:
			return il.AddExpr(LLIL_CMP_NE, size * 2, 0,
			    il.AddExpr(LLIL_LSR, size * 2, 0,
			        il.GetExprForRegisterOrConstantOperation(op, size, operands, operandCount),
			        il.AddExpr(LLIL_CONST, 1, 0, size * 8)),
			    il.AddExpr(LLIL_CONST, size * 2, 0, 0));
		}
	default:
		break;
	}
	if (((flagWriteType == IL_FLAGWRITE_X87COM) || (flagWriteType == IL_FLAGWRITE_X87C1Z)) &&
	    (flag == IL_FLAG_C1))
		return il.Const(0, 0);
	return Architecture::GetFlagWriteLowLevelIL(
	    op, size, flagWriteType, flag, operands, operandCount, il);
}

size_t X86CommonArchitecture::GetSemanticFlagGroupLowLevelIL(
...
{
	return vector<uint32_t> {IL_FLAG_C, IL_FLAG_P, IL_FLAG_A, IL_FLAG_Z, IL_FLAG_S, IL_FLAG_D,
	    IL_FLAG_O, IL_FLAG_C0, IL_FLAG_C1, IL_FLAG_C2, IL_FLAG_C3};
}
...
vector<uint32_t> X86CommonArchitecture::GetFlagsRequiredForFlagCondition(
...
map<uint32_t, BNLowLevelILFlagCondition>
    X86CommonArchitecture::GetFlagConditionsForSemanticFlagGroup(uint32_t semGroup)
{
	switch (semGroup)
	{
	case IL_FLAG_GROUP_E:
		return map<uint32_t, BNLowLevelILFlagCondition> {
		    {IL_FLAG_CLASS_INT, LLFC_E}, {IL_FLAG_CLASS_X87COMI, LLFC_FE}};
	case IL_FLAG_GROUP_NE:
		return map<uint32_t, BNLowLevelILFlagCondition> {
		    {IL_FLAG_CLASS_INT, LLFC_NE}, {IL_FLAG_CLASS_X87COMI, LLFC_FNE}};
	case IL_FLAG_GROUP_LT:
		return map<uint32_t, BNLowLevelILFlagCondition> {
		    {IL_FLAG_CLASS_INT, LLFC_ULT}, {IL_FLAG_CLASS_X87COMI, LLFC_FLT}};
	case IL_FLAG_GROUP_LE:
		return map<uint32_t, BNLowLevelILFlagCondition> {
		    {IL_FLAG_CLASS_INT, LLFC_ULE}, {IL_FLAG_CLASS_X87COMI, LLFC_FLE}};
	case IL_FLAG_GROUP_GE:
		return map<uint32_t, BNLowLevelILFlagCondition> {
		    {IL_FLAG_CLASS_INT, LLFC_UGE}, {IL_FLAG_CLASS_X87COMI, LLFC_FGE}};
	case IL_FLAG_GROUP_GT:
		return map<uint32_t, BNLowLevelILFlagCondition> {
		    {IL_FLAG_CLASS_INT, LLFC_UGT}, {IL_FLAG_CLASS_X87COMI, LLFC_FGT}};
	case IL_FLAG_GROUP_PE:
		return map<uint32_t, BNLowLevelILFlagCondition> {{IL_FLAG_CLASS_X87COMI, LLFC_FUO}};
	case IL_FLAG_GROUP_PO:
		return map<uint32_t, BNLowLevelILFlagCondition> {{IL_FLAG_CLASS_X87COMI, LLFC_FO}};
	default:
		return map<uint32_t, BNLowLevelILFlagCondition>();
	}
}
...
bool X86CommonArchitecture::Assemble(
    const string& code, uint64_t addr, DataBuffer& result, string& errors)
{
	...
	vector<string> args = vector<string> {
	    yasmPath, "-fbin", "-w", "-Worphan-labels", "-Werror", "-o", outputPath, inputPath};
...
bool X86CommonArchitecture::IsSkipAndReturnZeroPatchAvailable(
    const uint8_t* data, uint64_t, size_t len)
{
	xed_decoded_inst_t xedd;
	switch (m_bits)
	{
...
bool X86CommonArchitecture::IsSkipAndReturnValuePatchAvailable(
    const uint8_t* data, uint64_t, size_t len)
{
	xed_decoded_inst_t xedd;
	switch (m_bits)
...
class X86Architecture : public X86CommonArchitecture
{
 ...
 public:
	...
	virtual vector<uint32_t> GetFullWidthRegisters() override
	{
		return vector<uint32_t> {
		    // 16-Bit
		    XED_REG_CS, XED_REG_DS, XED_REG_ES, XED_REG_SS, XED_REG_FS, XED_REG_GS, XED_REG_FSBASE,
		    XED_REG_GSBASE,  // 16+
		    ...
		    XED_REG_CR0, XED_REG_CR1, XED_REG_CR2, XED_REG_CR3, XED_REG_CR4, XED_REG_CR5, XED_REG_CR6,
		    XED_REG_CR7, XED_REG_CR8, XED_REG_CR9, XED_REG_CR10, XED_REG_CR11, XED_REG_CR12,
		    XED_REG_CR13, XED_REG_CR14, XED_REG_CR15,  // 32+ (32 on 32, 64 on 64) Control Registers
		    XED_REG_DR0, XED_REG_DR1, XED_REG_DR2, XED_REG_DR3, XED_REG_DR4, XED_REG_DR5, XED_REG_DR6,
		    XED_REG_DR7,  // 32+ (starting in later revisions of 32) (32 on 32, 64 on 64) Debug
		                  // registers

		    XED_REG_MXCSR,  // 32+ SSE (MMX) Control Reg (32 on 32, 64 on 64)

		    // x87 FPU related
		    REG_X87_TOP, XED_REG_X87CONTROL, XED_REG_X87STATUS, XED_REG_X87TAG, XED_REG_X87OPCODE,
		    XED_REG_X87LASTCS, XED_REG_X87LASTDS, XED_REG_X87LASTIP, XED_REG_X87LASTDP, XED_REG_X87PUSH,
		    XED_REG_X87POP, XED_REG_X87POP2,
		    ...
		    // 80-Bit
		    XED_REG_ST0, XED_REG_ST1, XED_REG_ST2, XED_REG_ST3, XED_REG_ST4, XED_REG_ST5, XED_REG_ST6,
		    XED_REG_ST7,  // 32+ Floating point
		    REG_X87_r(0), REG_X87_r(1), REG_X87_r(2), REG_X87_r(3), REG_X87_r(4), REG_X87_r(5),
		    REG_X87_r(6), REG_X87_r(7),

		    // 128-Bit
		    XED_REG_XMM0, XED_REG_XMM1, XED_REG_XMM2, XED_REG_XMM3, XED_REG_XMM4, XED_REG_XMM5,
		    XED_REG_XMM6, XED_REG_XMM7,  // 32+ SSE
		};
	}
	...
	virtual vector<uint32_t> GetAllRegisters() override
	{
		return vector<uint32_t> {
		    ...
		    XED_REG_TSC,
		    XED_REG_TSCAUX,  // 32+ (32 on 32, 64 on 64) Timestamp Counters
		    XED_REG_TR,      // 16+ (16 on 16, 32 on 32, 64 on 64) Task Register
		    ...
		    XED_REG_CR13,
		    XED_REG_CR14,
		    XED_REG_CR15,  // 32+ (32 on 32, 64 on 64) Control Registers
		    XED_REG_DR0,
		    XED_REG_DR1,
		    XED_REG_DR2,
		    XED_REG_DR3,
		    ...
		    XED_REG_MMX5,
		    XED_REG_MMX6,
		    XED_REG_MMX7,  // 32+ Floating point, bottom of st regs
		    XED_REG_XCR0,  // 32+ (64 on 32, 64 on 64)
	...
	virtual BNRegisterInfo GetRegisterInfo(const uint32_t reg) override
	{
		...
		// 8-Bit
		case XED_REG_AH:
			return RegisterInfo(XED_REG_EAX, 1, 1);
		case XED_REG_CH:
			return RegisterInfo(XED_REG_ECX, 1, 1);
		case XED_REG_DH:
			return RegisterInfo(XED_REG_EDX, 1, 1);
		case XED_REG_BH:
			return RegisterInfo(XED_REG_EBX, 1, 1);
		case XED_REG_AL:
			return RegisterInfo(XED_REG_EAX, 0, 1);
		case XED_REG_CL:
			return RegisterInfo(XED_REG_ECX, 0, 1);
		case XED_REG_DL:
			return RegisterInfo(XED_REG_EDX, 0, 1);
		case XED_REG_BL:
			return RegisterInfo(XED_REG_EBX, 0, 1);

		// 16-Bit
		case XED_REG_IP:
			return RegisterInfo(XED_REG_EIP, 0, 2);

		case XED_REG_CS:
			return RegisterInfo(XED_REG_CS, 0, 2);
		case XED_REG_DS:
			return RegisterInfo(XED_REG_DS, 0, 2);
		case XED_REG_ES:
			return RegisterInfo(XED_REG_ES, 0, 2);
		case XED_REG_SS:
			return RegisterInfo(XED_REG_SS, 0, 2);
		case XED_REG_FS:
			return RegisterInfo(XED_REG_FS, 0, 2);
		case XED_REG_GS:
			return RegisterInfo(XED_REG_GS, 0, 2);

		case XED_REG_SP:
			return RegisterInfo(XED_REG_ESP, 0, 2);
		case XED_REG_BP:
			return RegisterInfo(XED_REG_EBP, 0, 2);
		case XED_REG_SI:
			return RegisterInfo(XED_REG_ESI, 0, 2);
		case XED_REG_DI:
			return RegisterInfo(XED_REG_EDI, 0, 2);

		case XED_REG_FLAGS:
			return RegisterInfo(XED_REG_EFLAGS, 0, 2);

		case XED_REG_AX:
			return RegisterInfo(XED_REG_EAX, 0, 2);
		case XED_REG_CX:
			return RegisterInfo(XED_REG_ECX, 0, 2);
		case XED_REG_DX:
			return RegisterInfo(XED_REG_EDX, 0, 2);
		case XED_REG_BX:
			return RegisterInfo(XED_REG_EBX, 0, 2);

		// 32-Bit
		case XED_REG_EIP:
			return RegisterInfo(XED_REG_EIP, 0, 4);
		case XED_REG_FSBASE:
			return RegisterInfo(XED_REG_FSBASE, 0, 4);
		case XED_REG_GSBASE:
			return RegisterInfo(XED_REG_GSBASE, 0, 4);

		case XED_REG_ESP:
			return RegisterInfo(XED_REG_ESP, 0, 4);
		case XED_REG_EBP:
			return RegisterInfo(XED_REG_EBP, 0, 4);
		case XED_REG_ESI:
			return RegisterInfo(XED_REG_ESI, 0, 4);
		case XED_REG_EDI:
			return RegisterInfo(XED_REG_EDI, 0, 4);

		case XED_REG_EFLAGS:
			return RegisterInfo(XED_REG_EFLAGS, 0, 4);

		case XED_REG_EAX:
			return RegisterInfo(XED_REG_EAX, 0, 4);
		case XED_REG_ECX:
			return RegisterInfo(XED_REG_ECX, 0, 4);
		case XED_REG_EDX:
			return RegisterInfo(XED_REG_EDX, 0, 4);
		case XED_REG_EBX:
			return RegisterInfo(XED_REG_EBX, 0, 4);

		case XED_REG_TSC:
			return RegisterInfo(XED_REG_TSC, 0, 4);
		case XED_REG_TSCAUX:
			return RegisterInfo(XED_REG_TSCAUX, 0, 4);

		case XED_REG_TR:
			return RegisterInfo(XED_REG_TR, 0, 4);

		case REG_X87_TOP:
			return RegisterInfo((xed_reg_enum_t)REG_X87_TOP, 0, 2);
		case XED_REG_X87CONTROL:
			return RegisterInfo(XED_REG_X87CONTROL, 0, 2);
		case XED_REG_X87STATUS:
			return RegisterInfo(XED_REG_X87STATUS, 0, 2);
		case XED_REG_X87TAG:
			return RegisterInfo(XED_REG_X87TAG, 0, 2);
		case XED_REG_X87OPCODE:
			return RegisterInfo(XED_REG_X87OPCODE, 0, 2);
		case XED_REG_X87LASTCS:
			return RegisterInfo(XED_REG_X87LASTCS, 0, 2);
		case XED_REG_X87LASTDS:
			return RegisterInfo(XED_REG_X87LASTDS, 0, 2);
		case XED_REG_X87LASTIP:
			return RegisterInfo(XED_REG_X87LASTIP, 0, 4);
		case XED_REG_X87LASTDP:
			return RegisterInfo(XED_REG_X87LASTDP, 0, 4);
		case XED_REG_X87PUSH:
			return RegisterInfo(XED_REG_X87PUSH, 0, 4);
		case XED_REG_X87POP:
			return RegisterInfo(XED_REG_X87POP, 0, 4);
		case XED_REG_X87POP2:
			return RegisterInfo(XED_REG_X87POP2, 0, 4);

		case XED_REG_CR0:
			return RegisterInfo(XED_REG_CR0, 0, 4);
		case XED_REG_CR1:
			return RegisterInfo(XED_REG_CR1, 0, 4);
		case XED_REG_CR2:
			return RegisterInfo(XED_REG_CR2, 0, 4);
		case XED_REG_CR3:
			return RegisterInfo(XED_REG_CR3, 0, 4);
		case XED_REG_CR4:
			return RegisterInfo(XED_REG_CR4, 0, 4);
		case XED_REG_CR5:
			return RegisterInfo(XED_REG_CR5, 0, 4);
		case XED_REG_CR6:
			return RegisterInfo(XED_REG_CR6, 0, 4);
		case XED_REG_CR7:
			return RegisterInfo(XED_REG_CR7, 0, 4);
		case XED_REG_CR8:
			return RegisterInfo(XED_REG_CR8, 0, 4);
		case XED_REG_CR9:
			return RegisterInfo(XED_REG_CR9, 0, 4);
		case XED_REG_CR10:
			return RegisterInfo(XED_REG_CR10, 0, 4);
		case XED_REG_CR11:
			return RegisterInfo(XED_REG_CR11, 0, 4);
		case XED_REG_CR12:
			return RegisterInfo(XED_REG_CR12, 0, 4);
		case XED_REG_CR13:
			return RegisterInfo(XED_REG_CR13, 0, 4);
		case XED_REG_CR14:
			return RegisterInfo(XED_REG_CR14, 0, 4);
		case XED_REG_CR15:
			return RegisterInfo(XED_REG_CR15, 0, 4);

		case XED_REG_DR0:
			return RegisterInfo(XED_REG_DR0, 0, 4);
		case XED_REG_DR1:
			return RegisterInfo(XED_REG_DR1, 0, 4);
		case XED_REG_DR2:
			return RegisterInfo(XED_REG_DR2, 0, 4);
		case XED_REG_DR3:
			return RegisterInfo(XED_REG_DR3, 0, 4);
		case XED_REG_DR4:
			return RegisterInfo(XED_REG_DR4, 0, 4);
		case XED_REG_DR5:
			return RegisterInfo(XED_REG_DR5, 0, 4);
		case XED_REG_DR6:
			return RegisterInfo(XED_REG_DR6, 0, 4);
		case XED_REG_DR7:
			return RegisterInfo(XED_REG_DR7, 0, 4);

		case XED_REG_MXCSR:
			return RegisterInfo(XED_REG_MXCSR, 0, 4);

		// 48-Bit
		case XED_REG_GDTR:
			return RegisterInfo(XED_REG_GDTR, 0, 6);
		case XED_REG_LDTR:
			return RegisterInfo(XED_REG_LDTR, 0, 6);
		case XED_REG_IDTR:
			return RegisterInfo(XED_REG_IDTR, 0, 6);

		// 64-Bit
		case XED_REG_MMX0:
			return RegisterInfo(XED_REG_ST0, 0, 8);
		case XED_REG_MMX1:
			return RegisterInfo(XED_REG_ST1, 0, 8);
		case XED_REG_MMX2:
			return RegisterInfo(XED_REG_ST2, 0, 8);
		case XED_REG_MMX3:
			return RegisterInfo(XED_REG_ST3, 0, 8);
		case XED_REG_MMX4:
			return RegisterInfo(XED_REG_ST4, 0, 8);
		case XED_REG_MMX5:
			return RegisterInfo(XED_REG_ST5, 0, 8);
		case XED_REG_MMX6:
			return RegisterInfo(XED_REG_ST6, 0, 8);
		case XED_REG_MMX7:
			return RegisterInfo(XED_REG_ST7, 0, 8);

		case XED_REG_XCR0:
			return RegisterInfo(XED_REG_XCR0, 0, 8);

		case XED_REG_RFLAGS:
			return RegisterInfo(XED_REG_RFLAGS, 0, 8);

		case XED_REG_MSRS:
			return RegisterInfo(XED_REG_MSRS, 0, 8);

		// 80-Bit
		case XED_REG_ST0:
			return RegisterInfo(XED_REG_ST0, 0, 10);
		case XED_REG_ST1:
			return RegisterInfo(XED_REG_ST1, 0, 10);
		case XED_REG_ST2:
			return RegisterInfo(XED_REG_ST2, 0, 10);
		case XED_REG_ST3:
			return RegisterInfo(XED_REG_ST3, 0, 10);
		case XED_REG_ST4:
			return RegisterInfo(XED_REG_ST4, 0, 10);
		case XED_REG_ST5:
			return RegisterInfo(XED_REG_ST5, 0, 10);
		case XED_REG_ST6:
			return RegisterInfo(XED_REG_ST6, 0, 10);
		case XED_REG_ST7:
			return RegisterInfo(XED_REG_ST7, 0, 10);

		case REG_X87_r(0):
			return RegisterInfo((xed_reg_enum_t)REG_X87_r(0), 0, 10);
		case REG_X87_r(1):
			return RegisterInfo((xed_reg_enum_t)REG_X87_r(1), 0, 10);
		case REG_X87_r(2):
			return RegisterInfo((xed_reg_enum_t)REG_X87_r(2), 0, 10);
		case REG_X87_r(3):
			return RegisterInfo((xed_reg_enum_t)REG_X87_r(3), 0, 10);
		case REG_X87_r(4):
			return RegisterInfo((xed_reg_enum_t)REG_X87_r(4), 0, 10);
		case REG_X87_r(5):
			return RegisterInfo((xed_reg_enum_t)REG_X87_r(5), 0, 10);
		case REG_X87_r(6):
			return RegisterInfo((xed_reg_enum_t)REG_X87_r(6), 0, 10);
		case REG_X87_r(7):
			return RegisterInfo((xed_reg_enum_t)REG_X87_r(7), 0, 10);

		// 128-Bit
		case XED_REG_XMM0:
			return RegisterInfo(XED_REG_XMM0, 0, 16);
		case XED_REG_XMM1:
			return RegisterInfo(XED_REG_XMM1, 0, 16);
		case XED_REG_XMM2:
			return RegisterInfo(XED_REG_XMM2, 0, 16);
		case XED_REG_XMM3:
			return RegisterInfo(XED_REG_XMM3, 0, 16);
		case XED_REG_XMM4:
			return RegisterInfo(XED_REG_XMM4, 0, 16);
		case XED_REG_XMM5:
			return RegisterInfo(XED_REG_XMM5, 0, 16);
		case XED_REG_XMM6:
			return RegisterInfo(XED_REG_XMM6, 0, 16);
		case XED_REG_XMM7:
			return RegisterInfo(XED_REG_XMM7, 0, 16);

		case XED_REG_BND0:
			return RegisterInfo(XED_REG_BND0, 0, 16);
		case XED_REG_BND1:
			return RegisterInfo(XED_REG_BND1, 0, 16);
		case XED_REG_BND2:
			return RegisterInfo(XED_REG_BND2, 0, 16);
		case XED_REG_BND3:
			return RegisterInfo(XED_REG_BND3, 0, 16);
	...
	virtual uint32_t GetStackPointerRegister() override { return XED_REG_ESP; }
};
...
{
 protected:
	virtual size_t GetAddressSize() const override { return 2; }
 ...
 public:
	...
	virtual vector<uint32_t> GetAllRegisters() override
	{
		return vector<uint32_t> {
		    // 8-Bit
		    XED_REG_AH, XED_REG_CH, XED_REG_DH, XED_REG_BH, XED_REG_AL, XED_REG_CL, XED_REG_DL,
		    XED_REG_BL,  // 16+
	...
	virtual BNRegisterInfo GetRegisterInfo(const uint32_t reg) override
	{
		...
		// 8-Bit
		case XED_REG_AH:
			return RegisterInfo(XED_REG_AX, 1, 1);
		case XED_REG_CH:
			return RegisterInfo(XED_REG_CX, 1, 1);
		case XED_REG_DH:
			return RegisterInfo(XED_REG_DX, 1, 1);
		case XED_REG_BH:
			return RegisterInfo(XED_REG_BX, 1, 1);
		case XED_REG_AL:
			return RegisterInfo(XED_REG_AX, 0, 1);
		case XED_REG_CL:
			return RegisterInfo(XED_REG_CX, 0, 1);
		case XED_REG_DL:
			return RegisterInfo(XED_REG_DX, 0, 1);
		case XED_REG_BL:
			return RegisterInfo(XED_REG_BX, 0, 1);

		// 16-Bit
		case XED_REG_IP:
			return RegisterInfo(XED_REG_IP, 0, 2);
		case XED_REG_CS:
			return RegisterInfo(XED_REG_CS, 0, 2);
		case XED_REG_DS:
			return RegisterInfo(XED_REG_DS, 0, 2);
		...
		case XED_REG_GS:
			return RegisterInfo(XED_REG_GS, 0, 2);
		case XED_REG_FSBASE:
			return RegisterInfo(XED_REG_FSBASE, 0, 2);
		case XED_REG_GSBASE:
			return RegisterInfo(XED_REG_GSBASE, 0, 2);
		case XED_REG_SP:
			return RegisterInfo(XED_REG_SP, 0, 2);
		case XED_REG_BP:
			return RegisterInfo(XED_REG_BP, 0, 2);
		case XED_REG_SI:
			return RegisterInfo(XED_REG_SI, 0, 2);
		case XED_REG_DI:
			return RegisterInfo(XED_REG_DI, 0, 2);
		case XED_REG_FLAGS:
			return RegisterInfo(XED_REG_FLAGS, 0, 2);
		case XED_REG_AX:
			return RegisterInfo(XED_REG_AX, 0, 2);
		case XED_REG_CX:
			return RegisterInfo(XED_REG_CX, 0, 2);
		case XED_REG_DX:
			return RegisterInfo(XED_REG_DX, 0, 2);
		case XED_REG_BX:
			return RegisterInfo(XED_REG_BX, 0, 2);
		case XED_REG_TR:
			return RegisterInfo(XED_REG_TR, 0, 2);
	...
	virtual uint32_t GetStackPointerRegister() override { return XED_REG_SP; }
};
 ...
 public:
	...
	virtual vector<uint32_t> GetFullWidthRegisters() override
	{
		return vector<uint32_t> {
		    ...
		    XED_REG_K0, XED_REG_K1, XED_REG_K2, XED_REG_K3, XED_REG_K4, XED_REG_K5, XED_REG_K6,
		    XED_REG_K7,  // 64+ AVX bit-masking registers (also not confident in size)
		    ...
		    XED_REG_ZMM30, XED_REG_ZMM31  // 64+ AVX
		};
	}
	...
	virtual vector<uint32_t> GetAllRegisters() override
	{
		return vector<uint32_t> {
		    ...
		    XED_REG_R8B, XED_REG_R9B, XED_REG_R10B, XED_REG_R11B, XED_REG_R12B, XED_REG_R13B,
		    XED_REG_R14B, XED_REG_R15B,  // 64+
		    ...
		    XED_REG_X87POP2, XED_REG_X87OPCODE, XED_REG_X87LASTCS, XED_REG_X87LASTIP, XED_REG_X87LASTDS,
		    XED_REG_X87LASTDP,

		    XED_REG_AX, XED_REG_CX, XED_REG_DX, XED_REG_BX,  // 16+
		    XED_REG_R8W, XED_REG_R9W, XED_REG_R10W, XED_REG_R11W, XED_REG_R12W, XED_REG_R13W,
		    XED_REG_R14W, XED_REG_R15W,  // 64+
		    ...
		    XED_REG_EAX, XED_REG_ECX, XED_REG_EDX, XED_REG_EBX,  // 32+
		    XED_REG_R8D, XED_REG_R9D, XED_REG_R10D, XED_REG_R11D, XED_REG_R12D, XED_REG_R13D,
		    XED_REG_R14D, XED_REG_R15D,  // 64+
		    ...
		    XED_REG_TR,                   // 16+ (16 on 16, 32 on 32, 64 on 64) Task Register
		    XED_REG_TSC, XED_REG_TSCAUX,  // 32+ (32 on 32, 64 on 64) Timestamp Counters
		    XED_REG_MMX0, XED_REG_MMX1, XED_REG_MMX2, XED_REG_MMX3, XED_REG_MMX4, XED_REG_MMX5,
		    XED_REG_MMX6, XED_REG_MMX7,  // 32+ Floating point, bottom of st regs
		    XED_REG_RIP,                 // 64+
		    XED_REG_RSP, XED_REG_RBP, XED_REG_RSI, XED_REG_RDI,  // 64+
		    XED_REG_RFLAGS,                                      // 64+
		    XED_REG_MXCSR,  // 32+ SSE (MMX) Control Reg (32 on 32, 64 on 64)
		    ...
		    XED_REG_XMM0, XED_REG_XMM1, XED_REG_XMM2, XED_REG_XMM3, XED_REG_XMM4, XED_REG_XMM5,
		    XED_REG_XMM6, XED_REG_XMM7,  // 32+ SSE
		    XED_REG_XMM8, XED_REG_XMM9, XED_REG_XMM10, XED_REG_XMM11, XED_REG_XMM12, XED_REG_XMM13,
		    XED_REG_XMM14, XED_REG_XMM15,  // 64+ SSE
		    XED_REG_XMM16, XED_REG_XMM17, XED_REG_XMM18, XED_REG_XMM19, XED_REG_XMM20, XED_REG_XMM21,
		    XED_REG_XMM22, XED_REG_XMM23, XED_REG_XMM24, XED_REG_XMM25, XED_REG_XMM26, XED_REG_XMM27,
		    XED_REG_XMM28, XED_REG_XMM29, XED_REG_XMM30, XED_REG_XMM31,  // 64+ AVX
		    XED_REG_BND0, XED_REG_BND1, XED_REG_BND2, XED_REG_BND3,      // 64 briefly. MPX registers
		    ...
		    XED_REG_YMM30, XED_REG_YMM31,  // 64+ AVX
	...
	virtual BNRegisterInfo GetRegisterInfo(const uint32_t reg) override
	{
		...
		// 8-Bit
		case XED_REG_AH:
			return RegisterInfo(XED_REG_RAX, 1, 1);
		case XED_REG_CH:
			return RegisterInfo(XED_REG_RCX, 1, 1);
		case XED_REG_DH:
			return RegisterInfo(XED_REG_RDX, 1, 1);
		case XED_REG_BH:
			return RegisterInfo(XED_REG_RBX, 1, 1);
		case XED_REG_AL:
			return RegisterInfo(XED_REG_RAX, 0, 1);
		case XED_REG_CL:
			return RegisterInfo(XED_REG_RCX, 0, 1);
		case XED_REG_DL:
			return RegisterInfo(XED_REG_RDX, 0, 1);
		case XED_REG_BL:
			return RegisterInfo(XED_REG_RBX, 0, 1);

		case XED_REG_SPL:
			return RegisterInfo(XED_REG_RSP, 0, 1);
		case XED_REG_BPL:
			return RegisterInfo(XED_REG_RBP, 0, 1);
		case XED_REG_SIL:
			return RegisterInfo(XED_REG_RSI, 0, 1);
		case XED_REG_DIL:
			return RegisterInfo(XED_REG_RDI, 0, 1);

		case XED_REG_R8B:
			return RegisterInfo(XED_REG_R8, 0, 1);
		case XED_REG_R9B:
			return RegisterInfo(XED_REG_R9, 0, 1);
		case XED_REG_R10B:
			return RegisterInfo(XED_REG_R10, 0, 1);
		case XED_REG_R11B:
			return RegisterInfo(XED_REG_R11, 0, 1);
		case XED_REG_R12B:
			return RegisterInfo(XED_REG_R12, 0, 1);
		case XED_REG_R13B:
			return RegisterInfo(XED_REG_R13, 0, 1);
		case XED_REG_R14B:
			return RegisterInfo(XED_REG_R14, 0, 1);
		case XED_REG_R15B:
			return RegisterInfo(XED_REG_R15, 0, 1);

		// 16-Bit
		case XED_REG_IP:
			return RegisterInfo(XED_REG_RIP, 0, 2);
		...
		case XED_REG_SP:
			return RegisterInfo(XED_REG_RSP, 0, 2);
		case XED_REG_BP:
			return RegisterInfo(XED_REG_RBP, 0, 2);
		case XED_REG_SI:
			return RegisterInfo(XED_REG_RSI, 0, 2);
		case XED_REG_DI:
			return RegisterInfo(XED_REG_RDI, 0, 2);

		case XED_REG_FLAGS:
			return RegisterInfo(XED_REG_RFLAGS, 0, 2);

		case XED_REG_AX:
			return RegisterInfo(XED_REG_RAX, 0, 2);
		case XED_REG_CX:
			return RegisterInfo(XED_REG_RCX, 0, 2);
		case XED_REG_DX:
			return RegisterInfo(XED_REG_RDX, 0, 2);
		case XED_REG_BX:
			return RegisterInfo(XED_REG_RBX, 0, 2);
		case XED_REG_R8W:
			return RegisterInfo(XED_REG_R8, 0, 2);
		case XED_REG_R9W:
			return RegisterInfo(XED_REG_R9, 0, 2);
		case XED_REG_R10W:
			return RegisterInfo(XED_REG_R10, 0, 2);
		case XED_REG_R11W:
			return RegisterInfo(XED_REG_R11, 0, 2);
		case XED_REG_R12W:
			return RegisterInfo(XED_REG_R12, 0, 2);
		case XED_REG_R13W:
			return RegisterInfo(XED_REG_R13, 0, 2);
		case XED_REG_R14W:
			return RegisterInfo(XED_REG_R14, 0, 2);
		case XED_REG_R15W:
			return RegisterInfo(XED_REG_R15, 0, 2);

		// 32-Bit
		case XED_REG_EIP:
			return RegisterInfo(XED_REG_RIP, 0, 4);

		case XED_REG_ESP:
			return RegisterInfo(XED_REG_RSP, 0, 4, true);
		case XED_REG_EBP:
			return RegisterInfo(XED_REG_RBP, 0, 4, true);
		case XED_REG_ESI:
			return RegisterInfo(XED_REG_RSI, 0, 4, true);
		case XED_REG_EDI:
			return RegisterInfo(XED_REG_RDI, 0, 4, true);
		...
		case XED_REG_X87LASTDS:
			return RegisterInfo(XED_REG_X87LASTDS, 0, 2);
		case XED_REG_X87LASTIP:
			return RegisterInfo(XED_REG_X87LASTIP, 0, 8);
		case XED_REG_X87LASTDP:
			return RegisterInfo(XED_REG_X87LASTDP, 0, 8);
		case XED_REG_X87PUSH:
			return RegisterInfo(XED_REG_X87PUSH, 0, 8);
		case XED_REG_X87POP:
			return RegisterInfo(XED_REG_X87POP, 0, 8);
		case XED_REG_X87POP2:
			return RegisterInfo(XED_REG_X87POP2, 0, 8);

		case XED_REG_EFLAGS:
			return RegisterInfo(XED_REG_RFLAGS, 0, 4, true);

		case XED_REG_EAX:
			return RegisterInfo(XED_REG_RAX, 0, 4, true);
		case XED_REG_ECX:
			return RegisterInfo(XED_REG_RCX, 0, 4, true);
		case XED_REG_EDX:
			return RegisterInfo(XED_REG_RDX, 0, 4, true);
		case XED_REG_EBX:
			return RegisterInfo(XED_REG_RBX, 0, 4, true);

		case XED_REG_R8D:
			return RegisterInfo(XED_REG_R8, 0, 4, true);
		case XED_REG_R9D:
			return RegisterInfo(XED_REG_R9, 0, 4, true);
		case XED_REG_R10D:
			return RegisterInfo(XED_REG_R10, 0, 4, true);
		case XED_REG_R11D:
			return RegisterInfo(XED_REG_R11, 0, 4, true);
		case XED_REG_R12D:
			return RegisterInfo(XED_REG_R12, 0, 4, true);
		case XED_REG_R13D:
			return RegisterInfo(XED_REG_R13, 0, 4, true);
		case XED_REG_R14D:
			return RegisterInfo(XED_REG_R14, 0, 4, true);
		case XED_REG_R15D:
			return RegisterInfo(XED_REG_R15, 0, 4, true);
		...
		// 64-Bit
		case XED_REG_FSBASE:
			return RegisterInfo(XED_REG_FSBASE, 0, 8);
		case XED_REG_GSBASE:
			return RegisterInfo(XED_REG_GSBASE, 0, 8);
		case XED_REG_TSC:
			return RegisterInfo(XED_REG_TSC, 0, 8);
		case XED_REG_TSCAUX:
			return RegisterInfo(XED_REG_TSCAUX, 0, 8);
		case XED_REG_TR:
			return RegisterInfo(XED_REG_TR, 0, 8);
		...
		case XED_REG_RIP:
			return RegisterInfo(XED_REG_RIP, 0, 8);

		case XED_REG_RSP:
			return RegisterInfo(XED_REG_RSP, 0, 8);
		case XED_REG_RBP:
			return RegisterInfo(XED_REG_RBP, 0, 8);
		case XED_REG_RSI:
			return RegisterInfo(XED_REG_RSI, 0, 8);
		case XED_REG_RDI:
			return RegisterInfo(XED_REG_RDI, 0, 8);

		case XED_REG_RFLAGS:
			return RegisterInfo(XED_REG_RFLAGS, 0, 8);

		case XED_REG_MXCSR:
			return RegisterInfo(XED_REG_MXCSR, 0, 8);
		case XED_REG_XCR0:
			return RegisterInfo(XED_REG_XCR0, 0, 8);
		case XED_REG_SSP:
			return RegisterInfo(XED_REG_SSP, 0, 8);

		case XED_REG_RAX:
			return RegisterInfo(XED_REG_RAX, 0, 8);
		case XED_REG_RCX:
			return RegisterInfo(XED_REG_RCX, 0, 8);
		case XED_REG_RDX:
			return RegisterInfo(XED_REG_RDX, 0, 8);
		case XED_REG_RBX:
			return RegisterInfo(XED_REG_RBX, 0, 8);
		case XED_REG_R8:
			return RegisterInfo(XED_REG_R8, 0, 8);
		case XED_REG_R9:
			return RegisterInfo(XED_REG_R9, 0, 8);
		case XED_REG_R10:
			return RegisterInfo(XED_REG_R10, 0, 8);
		case XED_REG_R11:
			return RegisterInfo(XED_REG_R11, 0, 8);
		case XED_REG_R12:
			return RegisterInfo(XED_REG_R12, 0, 8);
		case XED_REG_R13:
			return RegisterInfo(XED_REG_R13, 0, 8);
		case XED_REG_R14:
			return RegisterInfo(XED_REG_R14, 0, 8);
		case XED_REG_R15:
			return RegisterInfo(XED_REG_R15, 0, 8);

		case XED_REG_BNDCFGU:
			return RegisterInfo(XED_REG_BNDCFGU, 0, 8);
		case XED_REG_BNDSTATUS:
			return RegisterInfo(XED_REG_BNDSTATUS, 0, 8);

		case XED_REG_K0:
			return RegisterInfo(XED_REG_K0, 0, 8);
		case XED_REG_K1:
			return RegisterInfo(XED_REG_K1, 0, 8);
		case XED_REG_K2:
			return RegisterInfo(XED_REG_K2, 0, 8);
		case XED_REG_K3:
			return RegisterInfo(XED_REG_K3, 0, 8);
		case XED_REG_K4:
			return RegisterInfo(XED_REG_K4, 0, 8);
		case XED_REG_K5:
			return RegisterInfo(XED_REG_K5, 0, 8);
		case XED_REG_K6:
			return RegisterInfo(XED_REG_K6, 0, 8);
		case XED_REG_K7:
			return RegisterInfo(XED_REG_K7, 0, 8);

		case XED_REG_CR0:
			return RegisterInfo(XED_REG_CR0, 0, 8);
		case XED_REG_CR1:
			return RegisterInfo(XED_REG_CR1, 0, 8);
		case XED_REG_CR2:
			return RegisterInfo(XED_REG_CR2, 0, 8);
		case XED_REG_CR3:
			return RegisterInfo(XED_REG_CR3, 0, 8);
		case XED_REG_CR4:
			return RegisterInfo(XED_REG_CR4, 0, 8);
		case XED_REG_CR5:
			return RegisterInfo(XED_REG_CR5, 0, 8);
		case XED_REG_CR6:
			return RegisterInfo(XED_REG_CR6, 0, 8);
		case XED_REG_CR7:
			return RegisterInfo(XED_REG_CR7, 0, 8);
		case XED_REG_CR8:
			return RegisterInfo(XED_REG_CR8, 0, 8);
		case XED_REG_CR9:
			return RegisterInfo(XED_REG_CR9, 0, 8);
		case XED_REG_CR10:
			return RegisterInfo(XED_REG_CR10, 0, 8);
		case XED_REG_CR11:
			return RegisterInfo(XED_REG_CR11, 0, 8);
		case XED_REG_CR12:
			return RegisterInfo(XED_REG_CR12, 0, 8);
		case XED_REG_CR13:
			return RegisterInfo(XED_REG_CR13, 0, 8);
		case XED_REG_CR14:
			return RegisterInfo(XED_REG_CR14, 0, 8);
		case XED_REG_CR15:
			return RegisterInfo(XED_REG_CR15, 0, 8);

		case XED_REG_DR0:
			return RegisterInfo(XED_REG_DR0, 0, 8);
		case XED_REG_DR1:
			return RegisterInfo(XED_REG_DR1, 0, 8);
		case XED_REG_DR2:
			return RegisterInfo(XED_REG_DR2, 0, 8);
		case XED_REG_DR3:
			return RegisterInfo(XED_REG_DR3, 0, 8);
		case XED_REG_DR4:
			return RegisterInfo(XED_REG_DR4, 0, 8);
		case XED_REG_DR5:
			return RegisterInfo(XED_REG_DR5, 0, 8);
		case XED_REG_DR6:
			return RegisterInfo(XED_REG_DR6, 0, 8);
		case XED_REG_DR7:
			return RegisterInfo(XED_REG_DR7, 0, 8);
		...
		// 128-Bit
		case XED_REG_XMM0:
			return RegisterInfo(XED_REG_ZMM0, 0, 16);
		case XED_REG_XMM1:
			return RegisterInfo(XED_REG_ZMM1, 0, 16);
		case XED_REG_XMM2:
			return RegisterInfo(XED_REG_ZMM2, 0, 16);
		case XED_REG_XMM3:
			return RegisterInfo(XED_REG_ZMM3, 0, 16);
		case XED_REG_XMM4:
			return RegisterInfo(XED_REG_ZMM4, 0, 16);
		case XED_REG_XMM5:
			return RegisterInfo(XED_REG_ZMM5, 0, 16);
		case XED_REG_XMM6:
			return RegisterInfo(XED_REG_ZMM6, 0, 16);
		case XED_REG_XMM7:
			return RegisterInfo(XED_REG_ZMM7, 0, 16);
		case XED_REG_XMM8:
			return RegisterInfo(XED_REG_ZMM8, 0, 16);
		case XED_REG_XMM9:
			return RegisterInfo(XED_REG_ZMM9, 0, 16);
		case XED_REG_XMM10:
			return RegisterInfo(XED_REG_ZMM10, 0, 16);
		case XED_REG_XMM11:
			return RegisterInfo(XED_REG_ZMM11, 0, 16);
		case XED_REG_XMM12:
			return RegisterInfo(XED_REG_ZMM12, 0, 16);
		case XED_REG_XMM13:
			return RegisterInfo(XED_REG_ZMM13, 0, 16);
		case XED_REG_XMM14:
			return RegisterInfo(XED_REG_ZMM14, 0, 16);
		case XED_REG_XMM15:
			return RegisterInfo(XED_REG_ZMM15, 0, 16);
		case XED_REG_XMM16:
			return RegisterInfo(XED_REG_ZMM16, 0, 16);
		case XED_REG_XMM17:
			return RegisterInfo(XED_REG_ZMM17, 0, 16);
		case XED_REG_XMM18:
			return RegisterInfo(XED_REG_ZMM18, 0, 16);
		case XED_REG_XMM19:
			return RegisterInfo(XED_REG_ZMM19, 0, 16);
		case XED_REG_XMM20:
			return RegisterInfo(XED_REG_ZMM20, 0, 16);
		case XED_REG_XMM21:
			return RegisterInfo(XED_REG_ZMM21, 0, 16);
		case XED_REG_XMM22:
			return RegisterInfo(XED_REG_ZMM22, 0, 16);
		case XED_REG_XMM23:
			return RegisterInfo(XED_REG_ZMM23, 0, 16);
		case XED_REG_XMM24:
			return RegisterInfo(XED_REG_ZMM24, 0, 16);
		case XED_REG_XMM25:
			return RegisterInfo(XED_REG_ZMM25, 0, 16);
		case XED_REG_XMM26:
			return RegisterInfo(XED_REG_ZMM26, 0, 16);
		case XED_REG_XMM27:
			return RegisterInfo(XED_REG_ZMM27, 0, 16);
		case XED_REG_XMM28:
			return RegisterInfo(XED_REG_ZMM28, 0, 16);
		case XED_REG_XMM29:
			return RegisterInfo(XED_REG_ZMM29, 0, 16);
		case XED_REG_XMM30:
			return RegisterInfo(XED_REG_ZMM30, 0, 16);
		case XED_REG_XMM31:
			return RegisterInfo(XED_REG_ZMM31, 0, 16);
		...
		// 256-Bit
		case XED_REG_YMM0:
			return RegisterInfo(XED_REG_ZMM0, 0, 32);
		case XED_REG_YMM1:
			return RegisterInfo(XED_REG_ZMM1, 0, 32);
		case XED_REG_YMM2:
			return RegisterInfo(XED_REG_ZMM2, 0, 32);
		case XED_REG_YMM3:
			return RegisterInfo(XED_REG_ZMM3, 0, 32);
		case XED_REG_YMM4:
			return RegisterInfo(XED_REG_ZMM4, 0, 32);
		case XED_REG_YMM5:
			return RegisterInfo(XED_REG_ZMM5, 0, 32);
		case XED_REG_YMM6:
			return RegisterInfo(XED_REG_ZMM6, 0, 32);
		case XED_REG_YMM7:
			return RegisterInfo(XED_REG_ZMM7, 0, 32);
		case XED_REG_YMM8:
			return RegisterInfo(XED_REG_ZMM8, 0, 32);
		case XED_REG_YMM9:
			return RegisterInfo(XED_REG_ZMM9, 0, 32);
		case XED_REG_YMM10:
			return RegisterInfo(XED_REG_ZMM10, 0, 32);
		case XED_REG_YMM11:
			return RegisterInfo(XED_REG_ZMM11, 0, 32);
		case XED_REG_YMM12:
			return RegisterInfo(XED_REG_ZMM12, 0, 32);
		case XED_REG_YMM13:
			return RegisterInfo(XED_REG_ZMM13, 0, 32);
		case XED_REG_YMM14:
			return RegisterInfo(XED_REG_ZMM14, 0, 32);
		case XED_REG_YMM15:
			return RegisterInfo(XED_REG_ZMM15, 0, 32);
		case XED_REG_YMM16:
			return RegisterInfo(XED_REG_ZMM16, 0, 32);
		case XED_REG_YMM17:
			return RegisterInfo(XED_REG_ZMM17, 0, 32);
		case XED_REG_YMM18:
			return RegisterInfo(XED_REG_ZMM18, 0, 32);
		case XED_REG_YMM19:
			return RegisterInfo(XED_REG_ZMM19, 0, 32);
		case XED_REG_YMM20:
			return RegisterInfo(XED_REG_ZMM20, 0, 32);
		case XED_REG_YMM21:
			return RegisterInfo(XED_REG_ZMM21, 0, 32);
		case XED_REG_YMM22:
			return RegisterInfo(XED_REG_ZMM22, 0, 32);
		case XED_REG_YMM23:
			return RegisterInfo(XED_REG_ZMM23, 0, 32);
		case XED_REG_YMM24:
			return RegisterInfo(XED_REG_ZMM24, 0, 32);
		case XED_REG_YMM25:
			return RegisterInfo(XED_REG_ZMM25, 0, 32);
		case XED_REG_YMM26:
			return RegisterInfo(XED_REG_ZMM26, 0, 32);
		case XED_REG_YMM27:
			return RegisterInfo(XED_REG_ZMM27, 0, 32);
		case XED_REG_YMM28:
			return RegisterInfo(XED_REG_ZMM28, 0, 32);
		case XED_REG_YMM29:
			return RegisterInfo(XED_REG_ZMM29, 0, 32);
		case XED_REG_YMM30:
			return RegisterInfo(XED_REG_ZMM30, 0, 32);
		case XED_REG_YMM31:
			return RegisterInfo(XED_REG_ZMM31, 0, 32);

		// 512-Bit
		case XED_REG_ZMM0:
			return RegisterInfo(XED_REG_ZMM0, 0, 64);
		case XED_REG_ZMM1:
			return RegisterInfo(XED_REG_ZMM1, 0, 64);
		case XED_REG_ZMM2:
			return RegisterInfo(XED_REG_ZMM2, 0, 64);
		case XED_REG_ZMM3:
			return RegisterInfo(XED_REG_ZMM3, 0, 64);
		case XED_REG_ZMM4:
			return RegisterInfo(XED_REG_ZMM4, 0, 64);
		case XED_REG_ZMM5:
			return RegisterInfo(XED_REG_ZMM5, 0, 64);
		case XED_REG_ZMM6:
			return RegisterInfo(XED_REG_ZMM6, 0, 64);
		case XED_REG_ZMM7:
			return RegisterInfo(XED_REG_ZMM7, 0, 64);
		case XED_REG_ZMM8:
			return RegisterInfo(XED_REG_ZMM8, 0, 64);
		case XED_REG_ZMM9:
			return RegisterInfo(XED_REG_ZMM9, 0, 64);
		case XED_REG_ZMM10:
			return RegisterInfo(XED_REG_ZMM10, 0, 64);
		case XED_REG_ZMM11:
			return RegisterInfo(XED_REG_ZMM11, 0, 64);
		case XED_REG_ZMM12:
			return RegisterInfo(XED_REG_ZMM12, 0, 64);
		case XED_REG_ZMM13:
			return RegisterInfo(XED_REG_ZMM13, 0, 64);
		case XED_REG_ZMM14:
			return RegisterInfo(XED_REG_ZMM14, 0, 64);
		case XED_REG_ZMM15:
			return RegisterInfo(XED_REG_ZMM15, 0, 64);
		case XED_REG_ZMM16:
			return RegisterInfo(XED_REG_ZMM16, 0, 64);
		case XED_REG_ZMM17:
			return RegisterInfo(XED_REG_ZMM17, 0, 64);
		case XED_REG_ZMM18:
			return RegisterInfo(XED_REG_ZMM18, 0, 64);
		case XED_REG_ZMM19:
			return RegisterInfo(XED_REG_ZMM19, 0, 64);
		case XED_REG_ZMM20:
			return RegisterInfo(XED_REG_ZMM20, 0, 64);
		case XED_REG_ZMM21:
			return RegisterInfo(XED_REG_ZMM21, 0, 64);
		case XED_REG_ZMM22:
			return RegisterInfo(XED_REG_ZMM22, 0, 64);
		case XED_REG_ZMM23:
			return RegisterInfo(XED_REG_ZMM23, 0, 64);
		case XED_REG_ZMM24:
			return RegisterInfo(XED_REG_ZMM24, 0, 64);
		case XED_REG_ZMM25:
			return RegisterInfo(XED_REG_ZMM25, 0, 64);
		case XED_REG_ZMM26:
			return RegisterInfo(XED_REG_ZMM26, 0, 64);
		case XED_REG_ZMM27:
			return RegisterInfo(XED_REG_ZMM27, 0, 64);
		case XED_REG_ZMM28:
			return RegisterInfo(XED_REG_ZMM28, 0, 64);
		case XED_REG_ZMM29:
			return RegisterInfo(XED_REG_ZMM29, 0, 64);
		case XED_REG_ZMM30:
			return RegisterInfo(XED_REG_ZMM30, 0, 64);
		case XED_REG_ZMM31:
			return RegisterInfo(XED_REG_ZMM31, 0, 64);
	...
	virtual uint32_t GetStackPointerRegister() override { return XED_REG_RSP; }
};
...
class X86BaseCallingConvention : public CallingConvention
{
 public:
	...
	virtual uint32_t GetGlobalPointerRegister() override { return XED_REG_EBX; }

	virtual uint32_t GetIntegerReturnValueRegister() override { return XED_REG_EAX; }

	virtual uint32_t GetHighIntegerReturnValueRegister() override { return XED_REG_EDX; }

	virtual uint32_t GetFloatReturnValueRegister() override { return XED_REG_ST0; }
...
class X86LinuxSystemCallConvention : public CallingConvention
{
 public:
	...
	{
		return vector<uint32_t> {
		    XED_REG_EAX, XED_REG_EBX, XED_REG_ECX, XED_REG_EDX, XED_REG_ESI, XED_REG_EDI, XED_REG_EBP};
	}
...
class X64BaseCallingConvention : public CallingConvention
{
 public:
	...
	virtual uint32_t GetIntegerReturnValueRegister() override { return XED_REG_RAX; }

	virtual uint32_t GetFloatReturnValueRegister() override { return XED_REG_XMM0; }
...
class X64SystemVCallingConvention : public X64BaseCallingConvention
{
 public:
	...
	{
		return vector<uint32_t> {
		    XED_REG_RDI, XED_REG_RSI, XED_REG_RDX, XED_REG_RCX, XED_REG_R8, XED_REG_R9};
	}

	virtual vector<uint32_t> GetFloatArgumentRegisters() override
	{
		return vector<uint32_t> {XED_REG_XMM0, XED_REG_XMM1, XED_REG_XMM2, XED_REG_XMM3, XED_REG_XMM4,
		    XED_REG_XMM5, XED_REG_XMM6, XED_REG_XMM7};
	}

	virtual vector<uint32_t> GetCallerSavedRegisters() override
	{
		return vector<uint32_t> {XED_REG_RAX, XED_REG_RCX, XED_REG_RDX, XED_REG_RSI, XED_REG_RDI,
		    XED_REG_R8, XED_REG_R9, XED_REG_R10, XED_REG_R11, XED_REG_XMM0, XED_REG_XMM1, XED_REG_XMM2,
		    XED_REG_XMM3, XED_REG_XMM4, XED_REG_XMM5, XED_REG_XMM6, XED_REG_XMM7, XED_REG_XMM8,
		    XED_REG_XMM9, XED_REG_XMM10, XED_REG_XMM11, XED_REG_XMM12, XED_REG_XMM13, XED_REG_XMM14,
		    XED_REG_XMM15};
	}
	...
	{
		return vector<uint32_t> {
		    XED_REG_RBX, XED_REG_RBP, XED_REG_R12, XED_REG_R13, XED_REG_R14, XED_REG_R15};
	}
};
...
class X64WindowsCallingConvention : public X64BaseCallingConvention
{
 public:
	...
	virtual vector<uint32_t> GetCallerSavedRegisters() override
	{
		return vector<uint32_t> {XED_REG_RAX, XED_REG_RCX, XED_REG_RDX, XED_REG_R8, XED_REG_R9,
		    XED_REG_R10, XED_REG_R11, XED_REG_XMM4, XED_REG_XMM5};
	}

	virtual vector<uint32_t> GetCalleeSavedRegisters() override
	{
		return vector<uint32_t> {XED_REG_RBX, XED_REG_RBP, XED_REG_RSI, XED_REG_RDI, XED_REG_R12,
		    XED_REG_R13, XED_REG_R14, XED_REG_R15};
	}

	virtual bool AreArgumentRegistersSharedIndex() override { return true; }
...
class X64LinuxSystemCallConvention : public CallingConvention
{
 public:
	...
	{
		return vector<uint32_t> {
		    XED_REG_RAX, XED_REG_RDI, XED_REG_RSI, XED_REG_RDX, XED_REG_R10, XED_REG_R8, XED_REG_R9};
	}
 ...
 public:
	    ...
	    uint8_t* dest, size_t len) override
	{
		...
		case GENERIC_RELOC_VANILLA:
			...
			case 4:
				*dest32 = (uint32_t)(target - pcRelAddr);
				break;
			default:
				break;
			}
	    ...
	    Ref<BinaryView> view, Ref<Architecture> arch, vector<BNRelocationInfo>& result) override
	{
		...
		for (auto& reloc : relocTypes)
			LogWarn("Unsupported Mach-O relocation type: %s",
			    GetRelocationString((Machox86RelocationType)reloc));
		return true;
	}
};
...
class x86ElfRelocationHandler : public RelocationHandler
{
 public:
	    ...
	    Ref<BinaryView> view, Ref<Architecture> arch, vector<BNRelocationInfo>& result) override
	{
		...
		for (auto& reloc : relocTypes)
			LogWarn(
			    "Unsupported ELF relocation type: %s", GetRelocationString((Elfx86RelocationType)reloc));
		return true;
	}
};
...
class x64MachoRelocationHandler : public RelocationHandler
{
 public:
	    ...
	    uint8_t* dest, size_t len) override
	{
		...
		case X86_64_RELOC_UNSIGNED:
			if (!info.external)
			{
				...
				{
				case 4:
					*dest32 += target - (uint32_t)pcRelAddr;
					break;
				case 8:
					*dest64 += info.target - pcRelAddr;
					break;
				default:
					break;
				}
	    ...
	    Ref<BinaryView> view, Ref<Architecture> arch, vector<BNRelocationInfo>& result) override
	{
		...
		for (auto& reloc : relocTypes)
			LogWarn(
			    "Unsupported Mach-O relocation: %s", GetRelocationString((Machox64RelocationType)reloc));
		return true;
	}
};
...
class x64ElfRelocationHandler : public RelocationHandler
{
 public:
	    ...
	    Ref<BinaryView> view, Ref<Architecture> arch, vector<BNRelocationInfo>& result) override
	{
		...
		B Represents the base address at which a shared object has been loaded into memory during
		execution. Generally, a shared object is built with a 0 base virtual address, but the execution
		address will be different. G Represents the offset into the global offset table at which the
		relocation entry’s symbol will reside during execution. GOT Represents the address of the global
		offset table. L Represents the place (section offset or address) of the Procedure Linkage Table
		  entry for a symbol.
		P Represents the place (section offset or address) of the storage unit being relocated (computed
		using r_offset). S Represents the value of the symbol whose index resides in the relocation
		entry. Z Represents the size of the symbol whose index resides in the relocation entry. The
		AMD64 LP64 ABI architecture uses only Elf64_Rela relocation entries with explicit addends. Theg
		...
		Relocations contained within executable files or shared objects may use either Elf32_Rela
		relocation or Elf32_Rel relocation.
		*/
		(void)view;
		(void)arch;
		set<uint64_t> relocTypes;
...
class PeRelocationHandler : public RelocationHandler
{
 public:
	virtual bool ApplyRelocation(Ref<BinaryView> view, Ref<Architecture> arch, Ref<Relocation> reloc,
	    uint8_t* dest, size_t len) override
	{
		// Note: info.base contains preferred base address and the base where the image is actually
		// loaded
		(void)view;
		(void)arch;
		(void)len;
